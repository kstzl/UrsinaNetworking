...
class TestObject(Button, Replicator):
    ...
    def input(self, key):
        if self.hovered and key == "left mouse down":
            rpc_server(self.destroy_server)
        if self.hovered and key == "right mouse down":
            rpc_server(self.place_server, self.position + mouse.normal)